...
//@ask: why are we using shared pointer as opposed to regular pointers?
//@ask: Does it make a difference in outcome?!?!?
...
class socketInfoClass {
    ...
    mutex socketInfoMutex; //@sina: mutex to lock the object
public:
...
socketInfoClassSp getDesInfoP(int des) {
    ...
    if (iter == desInfoMap.end()) //@sina: find was unsuccessful
...
ssize_t myWrite(int des, const void* buf, size_t nbyte) {
    shared_lock<shared_mutex> desInfoLk(mapMutex);
    socketInfoClassSp desInfoP = getDesInfoP(des);
    if(desInfoP != nullptr && desInfoP->pair >= 0) {
...
int myTcdrain(int des) {
    ...
    //@sina: The tcdrain() function shall block until all output written to the object referred to by fildes is transmitted.
    //@sina: The fildes argument is an open file descriptor associated with a terminal.
    /*@sina:So draining just means data being transmitted.  Think that when you write some data using the myWrite() function,
     * it is like dumping water into a sink, it takes time for that water to drain out of the sink, just like it takes time for data to be transmitted
     * at some speed across a transmission medium like a telephone line.
     */
    /*myTcdrain() is used so that the XMODEM sender, for example, will, in future parts of the project,
     * be able to wait for the medium to have received all the bytes of a block already written to the medium before
     * sending the last byte of the block.
     */
 ...
 */
//@sina: only closes the calling socket - not the pair
int myClose(int des) {
    int retVal = 1;
    lock_guard<shared_mutex> desInfoLk(mapMutex);
    auto iter = desInfoMap.find(des);
