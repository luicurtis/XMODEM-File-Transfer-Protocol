...
void ReceiverX::clearCan()
{
    ...
    while (PE(myReadcond(mediumD, &bytesToRead, 1, 1, TM_2CHAR*dSECS_PER_UNIT, TM_2CHAR*dSECS_PER_UNIT)) > 0) {
        if (bytesToRead != CAN){
            CON_OUT(consoleInId, bytesToRead);
            break;
        }
    }
...
{
    char byteToRead;
    // Keep reading the medium until there are no more bytes to read, then it will timeout after 1 second
    while (PE(myReadcond(mediumD, &byteToRead, 1, 0, TM_CHAR, TM_CHAR)) > 0) {
