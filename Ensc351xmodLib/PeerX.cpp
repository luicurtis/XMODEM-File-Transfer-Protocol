...
transferCommon(std::shared_ptr<StateMgr> mySM, bool reportInfoParam)
{
    ...
    while(mySM->isRunning()) {
        ...
        if (now >= absoluteTimeout) {
            // Set timeval to 0 and post timeout event
            tv.tv_usec = 0;
            tv.tv_sec = 0;
            mySM->postEvent(TM);
        }
            ...
            // If no des active, post timeout
            if(rv == 0){
                mySM->postEvent(TM);
            }
            else{
                if(FD_ISSET(consoleInId, &set) ){
                    char byteToRead[LINEMAX];
                    int numread = myReadcond(consoleInId, byteToRead, LINEMAX, 1, dSECS_PER_UNIT * TM_CHAR, dSECS_PER_UNIT * TM_CHAR);
                    byteToRead[numread] = '\0';

                    if(strcmp(byteToRead, CANC_C) == 0){
                        KbCan = true;
                        mySM->postEvent(KB_C);
                    }
                }
    ...
    //dumpGlitches();     // Dump glitches before closing to avoid errors
