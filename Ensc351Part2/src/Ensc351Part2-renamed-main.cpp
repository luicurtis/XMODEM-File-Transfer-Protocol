...
int myMain()
{
    ...
    PE(mySocketpair(AF_LOCAL, SOCK_STREAM, 0, daSktPrT1M));     // Create Skt Pair for receiver and med
    PE(mySocketpair(AF_LOCAL, SOCK_STREAM, 0, daSktPrMT2));     // Create Skt Pair for sender and med
    //daSktPr[Term1] =  PE(/*myO*/open("/dev/ser2", O_RDWR));

    thread term2Thrd(termFunc, Term2);
    // ***** create thread for medium *****
    thread mediumThrd(mediumFunc);      // Create kind medium thread
    termFunc(Term1);

    // ***** join with thread for medium *****
    term2Thrd.join();
    mediumThrd.join();      // Join the medium thread
