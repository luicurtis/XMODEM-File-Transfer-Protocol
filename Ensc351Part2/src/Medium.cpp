...
{
    ssize_t numRead = myRead(fildes, buf, nbyte );
    if (numRead == -1 && errno == 104) // errno 104 is "Connection reset by peer"
        numRead = 0; // switch errno 104 to 0 bytes read
    return numRead;
}
...
//	if (!(numOfByteReceived = PE(myRead(Term2D, bytesReceived, 1))))
	if (!(numOfByteReceived = PE(mediumRead(Term2D, bytesReceived, (BLK_SZ_CRC+1) / 2)))) {
